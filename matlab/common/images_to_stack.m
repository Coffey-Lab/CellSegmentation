...
for i= 1:length(PosList)
    ...
    imwrite(imread(DapiList{i}), [SlideDir '/Stacks/' OutPos{i} '_stack.tif']);
    %[s1 s2]=size(DapiImg);
    ...
    for j=1:(length(AFList))
        ...
        imwrite(imread([SlideDir '/AFRemoved/' AFList{j} '_AFRemoved_'  OutPos{i} '.tif']), [SlideDir '/Stacks/' OutPos{i} '_stack.tif'], 'writemode', 'append');
