function outstats = regionpropsETM(varargin)
%This is regionprops with the addition of median and integrated intensity
%measurements
...
for k = 1 : numRequestedStats
    switch requestedStats{k}
        ...
        case 'MedianIntensity'
            [stats, statsAlreadyComputed] = ...
                ComputeMedianIntensity(I,stats,statsAlreadyComputed);
        
        case 'StDevIntensity'
            [stats, statsAlreadyComputed] = ...
                ComputeStDevIntensity(I,stats,statsAlreadyComputed);
        ...
        case 'IntIntensity'
            [stats, statsAlreadyComputed] = ...
                ComputeIntIntensity(I,stats,statsAlreadyComputed);
    end
end
...
if ~statsAlreadyComputed.Centroid
    ...
    for k = 1:length(stats)
        stats(k).Centroid = round(mean(stats(k).PixelList,1));
    end
...
if ~(statsAlreadyComputed.MajorAxisLength && ...
    ...
    for k = 1:length(stats)
        ...
        if (isempty(list))
            ...
            stats(k).Eccentricity = round(2*sqrt((stats(k).MajorAxisLength/2)^2 - ...
                (stats(k).MinorAxisLength/2)^2) / ...
                stats(k).MajorAxisLength,3);
            ...
            if (num == 0) && (den == 0)
                ...
                stats(k).Orientation = round((180/pi) * atan(num/den),3);
            end
        end
    end
...
if ~statsAlreadyComputed.Solidity
    ...
    for k = 1:length(stats)
        if (stats(k).ConvexArea == 0)
            ...
            stats(k).Solidity = round(stats(k).Area / stats(k).ConvexArea,3);
        end
    end
end
...
if ~statsAlreadyComputed.MedianIntensity
    statsAlreadyComputed.MedianIntensity = 1;
    
    [stats, statsAlreadyComputed] = ...
        ComputePixelValues(I,stats,statsAlreadyComputed);
    
    for k = 1:length(stats)
        stats(k).MedianIntensity = round(nanmedian(stats(k).PixelValues));
    end
end

%%%
%%% ComputeStDevIntensity
%%%
function [stats, statsAlreadyComputed] = ...
    ComputeStDevIntensity(I,stats,statsAlreadyComputed)

if ~statsAlreadyComputed.StDevIntensity
    statsAlreadyComputed.StDevIntensity = 1;
    
    [stats, statsAlreadyComputed] = ...
        ComputePixelValues(I,stats,statsAlreadyComputed);
    
    for k = 1:length(stats)
        stats(k).StDevIntensity = round(std(double(stats(k).PixelValues)),3) ;
    end
end
...
if ~statsAlreadyComputed.IntIntensity
    statsAlreadyComputed.IntIntensity = 1;
    
    [stats, statsAlreadyComputed] = ...
        ComputePixelValues(I,stats,statsAlreadyComputed);
    
    for k = 1:length(stats)
        stats(k).IntIntensity = nansum(stats(k).PixelValues);
    end
end
